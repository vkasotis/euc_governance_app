...
def seed_database(force: bool = False) -> None:
    ...
    reviewers = ["DVU.Reviewer", "GCC.User", "DVU.Reviewer", "GCC.User"]
    ...
    legal_entities = ["Eurobank S.A.", "Eurobank S.A.", "Eurobank S.A.", "Eurobank S.A.", "Eurobank S.A."]
    euc_specs = [
        ...
        euc_id = create_euc(
            {
                ...
                "business_unit": unit,
                "legal_entity": legal_entities[idx % len(legal_entities)],
                "reviewer": reviewers[idx % len(reviewers)],
                "supports_material_report": "Yes" if idx % 2 else "No",
                "supports_material_kri": "Yes" if idx % 3 == 0 else "No",
                "supports_material_model": "Yes" if idx % 4 == 0 else "No",
                "technology_type": tech,
                "storage_location": f"//eurobank/euc/{unit.lower().replace(' ', '_')}/{idx+1}",
                "frequency": "Monthly" if idx % 3 else "Daily",
                "schedule": f"Working day {(idx % 10) + 1}",
                "cut_off": f"Working day {(idx % 10) + 2}",
            ...
            create_component(
                {
                    ...
                    "owner": owner,
                    "operationalization_document_link": f"https://sharepoint.local/{idx+1}/{n+1}",
                    "file_description": f"Seeded artifact for {name} part {n+1}",
                    "technology_type": tech,
                    "controlled_storage_type": "SharePoint" if n == 0 else "Network Drive",
                    "controlled_storage_location": f"//controlled/euc/components/{idx+1}/{n+1}",
                    "input_sources": "Source system A; Source system B",
                    "cut_off": f"Working day {(idx % 10) + 2}",
                    "processing_schedule": f"Working day {(idx % 10) + 1} 08:00 CET",
                    "execution_frequency": "Monthly" if idx % 3 else "Daily",
                    "cde_mappings": "Customer ID; Exposure" if n == 0 else "Account; Balance",
                    "data_outputs": "Management report output; control summary",
                    "level_of_automation": "Semi-automated" if n == 0 else "Manual",
                    "backup_recovery_arrangements": "Nightly backup; restore tested",
                    "spof_risk": "Yes" if scores[0] >= 4 and n == 0 else "No",
