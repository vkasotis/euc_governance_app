...
def page_register() -> None:
    ...
    with st.form("register_euc"):
        ...
        onboarding_type = st.selectbox("Onboarding type", ["New EUC", "Legacy EUC"], help="Legacy EUCs use a current-state evidence baseline; historical testing/UAT/approval evidence is not recreated solely for initial registration.")
        ...
        euc_operationalization_document_link = link1.text_input("EUC operating process / operationalization document link")
        policy242_operationalization_link = link2.text_input("Policy 242 RRF / material report operationalization link")
        design_logic_applicable = st.selectbox(
            "Does the EUC contain material business logic / automation / complex calculations not readily understood through the Operating Procedure alone?",
            ["No", "Yes"],
            help="If Yes, Design / Logic Documentation becomes required. If No, proportionate explanation in the Operating Procedure is generally sufficient.",
        )
        design_logic_rationale = st.text_area("Design / logic applicability rationale", help="Briefly explain the Yes/No answer for design/logic documentation applicability.")
            ...
            euc_id = svc.create_euc(
                {
                    ...
                    "design_logic_applicable": design_logic_applicable,
                    "design_logic_rationale": design_logic_rationale,
                    "euc_operationalization_document_link": euc_operationalization_document_link,
                    "policy242_operationalization_link": policy242_operationalization_link,
                    "inputs": inputs,
                    "outputs": outputs,
    ...
    with tabs[0]:
        ...
        record_table(
            ...
            [
                ...
                ("Design / logic documentation applicable", "design_logic_applicable"),
                ("BCBS 239 elevated inherent-risk override", "bcbs239_elevated_inherent_override"),
                "technology_type",
                "storage_location",
                "frequency",
        ...
        if svc.can_edit_euc(role, username, euc):
            with st.expander("Edit EUC summary and lifecycle"):
                with st.form("edit_euc"):
                    ...
                    onboarding_type = st.selectbox("Onboarding type", ["New EUC", "Legacy EUC"], index=option_index(["New EUC", "Legacy EUC"], euc.get("onboarding_type") or "New EUC"), help="Legacy EUCs use a current-state evidence baseline; historical testing/UAT/approval evidence is not recreated solely for initial registration.")
    ...
    with tabs[1]:
        record_table(
            ...
            [
                ...
                ("Design / logic documentation applicable", "design_logic_applicable"),
                ("Design / logic applicability rationale", "design_logic_rationale"),
                "inputs",
                "outputs",
                "recipients",
                "dependencies",
        ...
        if svc.can_edit_euc(role, username, euc):
            with st.expander("Edit mapping fields"):
                with st.form("edit_mapping"):
                    ...
                    l1, l2 = st.columns(2)
                    payload["euc_operationalization_document_link"] = l1.text_input("EUC operating process / operationalization document link", value=euc.get("euc_operationalization_document_link") or "")
                    payload["policy242_operationalization_link"] = l2.text_input("Policy 242 RRF / material report operationalization link", value=euc.get("policy242_operationalization_link") or "")
                    payload["design_logic_applicable"] = st.selectbox(
                        "Does the EUC contain material business logic / automation / complex calculations not readily understood through the Operating Procedure alone?",
                        ["No", "Yes"],
                        index=option_index(["No", "Yes"], euc.get("design_logic_applicable") or "No"),
                        key=f"detail_design_logic_applicable_{euc['euc_id']}",
                    )
                    payload["design_logic_rationale"] = st.text_area("Design / logic applicability rationale", value=euc.get("design_logic_rationale") or "")
...
def _component_asset_form(prefix: str, euc: dict[str, Any], component: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    cde_mappings = cde_linkage_multiselect("CDE Mappings", component.get("cde_mappings"), key=f"{prefix}_cde_mappings")
