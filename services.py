...
def artifact_upload_guidance(document_type: str) -> str:
    return ARTIFACT_UPLOAD_GUIDANCE.get(document_type, "Upload evidence sufficient for the reviewer to verify the requirement, control operation, owner, date, scope and conclusion.")
    ...
    for risk, docs in DEFAULT_REQUIRED_ARTIFACTS.items():
        for doc_type in docs:
            existing = fetch_one(
                ...
                WHERE risk_level = ? AND lifecycle_stage = 'Active' AND required_document_type = ? AND approval_status = 'Approved'
                LIMIT 1
                """,
                (risk, doc_type),
            )
            ...
            if "Access" in doc_type:
                control_area = "Access Control"
            elif "Testing" in doc_type or "UAT" in doc_type:
                control_area = "Data Validation"
            elif "Resilience" in doc_type:
                control_area = "Operational Resilience"
            cacrt = "Timeliness" if "Resilience" in doc_type else "Accuracy" if ("Testing" in doc_type or "UAT" in doc_type) else "Traceability"
            execute(
                """
                ...
                (
                    ...
                    cacrt,
                    1,
                    "Default policy rule loaded during initialization. Baseline is based on Overall Inherent Risk.",
                    username,
                    username,
                    "Approved",
                ),
...
def required_documents_for_euc(euc_id: int) -> pd.DataFrame:
    ...
    if not euc:
        return pd.DataFrame()
    # Policy basis: required artifacts are driven by Overall Inherent Risk.
    # Residual risk drives remediation / escalation / exception handling, but does not reduce the baseline evidence pack.
    risk = euc.get("inherent_risk") or "Medium"
    rules = dataframe(
        """
...
def artifact_checklist(euc_id: int) -> pd.DataFrame:
    ...
    for _, req in required.iterrows():
        ...
        if doc_type == "Risk Assessment":
            assessment = latest_risk_assessment(euc_id)
            if not assessment:
                status = "Missing"
            else:
                status = assessment.get("status") or "Submitted"
                assessment_id = int(assessment["assessment_id"])
                reviewed_by = assessment.get("reviewed_by")
                comments = (
                    f"Satisfied by risk assessment #{assessment_id}, version {assessment.get('version')}. "
...
def purge_euc_operational_data(username: str) -> dict[str, int]:
    """Delete EUC operational/demo records while preserving users/configuration/reference data/audit."""
    tables = [
        "notification_outbox",
        "material_changes",
        "incidents",
    ...
    # Remove uploaded evidence files but keep the folder.
    try:
        for path in UPLOAD_PATH.rglob("*"):
            if path.is_file():
                path.unlink()
        for path in sorted([p for p in UPLOAD_PATH.rglob("*") if p.is_dir()], reverse=True):
            path.rmdir()
    except Exception:
        pass
    set_app_setting("auto_seed_disabled", "1", username)
    insert_audit("System", "EUC_OPERATIONAL_DATA", "PURGE", username, None, deleted)
    return deleted
