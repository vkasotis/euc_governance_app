...
## Patch 25 regression restoration and controlled mapping update

This package restores the full patch23 feature set and re-applies the intended controlled mapping and risk-assessment enhancements without reverting to the smaller simplified implementation.

Key points:

- Risk Assessment remains the Excel-aligned model with BCBS 239 materiality questions, owner versus effective inherent risk, eight baseline controls, derived control effectiveness, dimension-level residual risk, status review, and version history.
- The `Registration & Risk Assessment` baseline control is guarded: it cannot be effective as `In place and evidenced` unless EUC registration/mapping is complete and a risk assessment has been accepted by an independent reviewer.
